...
export async function POST(req: NextRequest) {
  ...
  function filterByYear<T extends { url?: string }>(posts: T[]): T[] {
    return posts.filter(p => {
      ...
      if (year < minYear || year > maxYear) { filteredCount++; return false; }
      return true;
    });
  }

  const filteredResult = { local: filterByYear(result.local), global: filterByYear(result.global) };
...
export async function DELETE(req: NextRequest) {
  ...
  await sb.from('scraped_posts').delete()
    .eq('make_slug', makeSlug).eq('model_slug', modelSlug).eq('post_id', postId);

  const { data: existingStat } = await sb
    .from('scrape_source_stats').select('invalid_count')
    .eq('make_slug', makeSlug).eq('model_slug', modelSlug).eq('source_name', sourceName).single();

  if (existingStat) {
    await sb.from('scrape_source_stats')
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .update({ invalid_count: (existingStat as any).invalid_count + 1 })
...
export async function PATCH(req: NextRequest) {
  ...
  const sb = getServiceClient();
  await sb.from('scraped_posts').update({ cloned: true })
    .eq('make_slug', makeSlug).eq('model_slug', modelSlug).eq('post_id', postId);
  return NextResponse.json({ ok: true });
}
